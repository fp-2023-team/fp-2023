...
renderDataFrameAsTable givenTerminalWidth (DataFrame columns rows) =
  ...
  where
    ...
    columnWidths = reduceWidths unboundColumnWidths
      where
        ...
        reduceWidths widths
          ...
          | otherwise = map (`min` average) widths
        average = (terminalWidth `div` length columns) - ((length values) * 3)

    headerRow = renderRow $ zipWith renderColumnValue columns columnWidths
    separatorRow = separatorLine columnWidths
    dataRows = map (renderRow . (\row -> zipWith renderRowValue row columnWidths)) rows
    ...
    getValues columns' rows' = map (\i -> map (!! i) rows') [0 .. (length columns' - 1)] -- transpose matrix, so [Value] is column of values
    separatorLine :: [Int] -> String
    separatorLine widths = concatMap (\width -> widthToLine (width + 2) ++ "+") (init widths) ++ widthToLine (last widths + 2)
      where
        widthToLine width = replicate width '-'
    ...
    padAlignLeft str width = str ++ replicate (width - length str) ' '
