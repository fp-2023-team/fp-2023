...
runExecuteIO (Free step) = do
    ...
    where
        ...
        runStep (Lib3.SaveDatabase content next) = writeFile ("./db/database.json") content >>= return . next
        runStep (Lib3.LoadDatabase next) = readFile ("./db/database.json") >>= return . next
